...
# UVA Color Palette
uva_colors <- c(space_cadet = "#232d4b",
                indigo = "#2c4f6b",
                blue_munsell = "#0e879c",
                cadet_blue = "#60999a",
                tea_green = "#d1e0bf",
...
#DNAxFDA GRAPH FOR SHINY
ggplot(data = fda_corpfam_scatter, aes(x = (perc_fda * 100), y = (perc_dna *100))) +
  geom_point(stat = "identity") +
  scale_color_manual(values  = "#232d4b") +
  facet_grid(~year) +
  # geom_smooth(method='lm')
  #geom_text(data = fda_corpfam_scatter %>% filter(perc_dna > 0.01 |perc_fda > 0.01), aes(label = corporate.family )) +
  geom_text_repel(data = fda_corpfam_scatter %>% filter(perc_dna > 0.01 |perc_fda > 0.01), size = 2.5, aes(label = corporate.family), label.size = 0.1) +
  labs(y = '% DNA Mentions', x = '% FDA Listings', title = 'DNA and FDA percent listings') +
  theme_classic()

ggsave('../output/dna_fda_scatter.png')
...
#DNAxNDCxFDA graph for shiny
ggplot(data = ndc_corpfam_scatter_fdayn, aes(x = (perc_ndc * 100), y = (perc_dna * 100))) +
  geom_point(stat = "identity", aes(color = fda))+
  scale_color_manual(values  = c("#232d4b", "#e6a01d")) +
  facet_grid(~year) +
  #geom_text(data = ndc_corpfam_scatter_fdayn  %>% filter(corporate.family == 'cvs pharmacy'), aes(label = corporate.family)) +
  #geom_text(data = ndc_corpfam_scatter %>% filter(perc_dna > 0.01 |perc_ndc > 0.01), aes(label = corporate.family), size = 2.5, hjust = 0, nudge_x = 0.05) +
  geom_text_repel(data = ndc_corpfam_scatter %>% filter(perc_dna > 0.01 |perc_ndc > 0.01), size = 2.5, aes(label = corporate.family), label.size = 0.1) +
  labs(y = '% DNA Mentions', x = '% NDC Listings', colour = 'FDA Approval', title = "DNA and NDC percent listings") +
  theme_classic()

ggsave('../output/dna_ndc_scatter.png')

#Is there a way to make it dynamic, allow the user to choose which company to track over the year?
